...
### Added
- **评论联动计数** - 实现 `comment_count` 的创建/删除自动增减，并保持通知链路完整  
- **前端/后台调试 Token 生成** - 支持按严格 `user_type` 校验生成调试 Token  
- **用户关注/取关功能** - 实现完整关注链路，同步维护被关注计数与通知  

### Changed
- **上传/角色服务常量与校验逻辑** - 归置常量并精简重复校验逻辑  
- **文章/课程复制行为** - 停止复制文件关联资源及互动计数器  
- **头像与计数器存储** - 迁移至类型化属性系统（`string`/`bigint`），使用独立的用户/文章/课程属性表  

### Fixed
- **CI 流程稳定性** - 明确工作流审批门禁逻辑，移除无效的 CI 触发调整  
- **k6 API 测试套件** - 适配删除语义变更后稳定运行  
- **k6 API CI 稳定性** - 对预期非 2xx 响应进行分类，避免误判失败  
- **Playwright CRUD 测试** - 修复管理后台与小程序端不稳定用例，无需修改运行时或 Docker 配置  
- **Playwright UI 报表测试** - 移除硬编码模块外键依赖，解决失败问题  
- **文件类型校验** - 在 COS 下载签名前增加 `HeadObject` MIME 类型检查，强化校验安全性
