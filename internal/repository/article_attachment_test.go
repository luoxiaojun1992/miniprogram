...
var articleAttachmentColumns = []string{"id", "article_id", "file_id", "sort_order", "created_at"}

func TestNewArticleAttachmentRepository(t *testing.T) {
	db, _ := newTestDB(t)
	repo := NewArticleAttachmentRepository(db)
	require.NotNil(t, repo)
}

func TestArticleAttachmentRepository_ListFileIDs_Success(t *testing.T) {
	db, mock := newTestDB(t)
	repo := NewArticleAttachmentRepository(db)

	now := time.Now()
	mock.ExpectQuery("SELECT").WillReturnRows(
		sqlmock.NewRows(articleAttachmentColumns).
...
func TestArticleAttachmentRepository_ListFileIDs_Error(t *testing.T) {
	db, mock := newTestDB(t)
	repo := NewArticleAttachmentRepository(db)
...
func TestArticleAttachmentRepository_Replace_Success(t *testing.T) {
	db, mock := newTestDB(t)
	repo := NewArticleAttachmentRepository(db)
...
func TestArticleAttachmentRepository_Replace_DeleteError(t *testing.T) {
	db, mock := newTestDB(t)
	repo := NewArticleAttachmentRepository(db)
...
func TestArticleAttachmentRepository_Replace_CreateError(t *testing.T) {
	db, mock := newTestDB(t)
	repo := NewArticleAttachmentRepository(db)
...
func TestArticleAttachmentRepository_Replace_OnlyZeroIDs(t *testing.T) {
	db, mock := newTestDB(t)
	repo := NewArticleAttachmentRepository(db)
